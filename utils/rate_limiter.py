        ...
        with self._lock:
            ...
            if current_count < rate_limit_rpm:
                ...
                return True, {
                    ...
                    'reset_time': int(window_start + 60)  # When the oldest request will expire
                }
            else:
                # Rate limit exceeded
                return False, {
                    ...
                    'reset_time': int(request_queue[0] + 60)  # When the oldest request will expire
                }
    ...
    def get_stats(self, api_key: str) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                'current_count': current_count,
                'remaining': max(0, rate_limit_rpm - current_count) if rate_limit_rpm > 0 else 'unlimited',
                'reset_time': int(request_queue[0] + 60) if request_queue else int(current_time + 60)
            }
...
def rate_limit_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if hasattr(response, 'headers'):
            response.headers['X-RateLimit-Limit'] = str(rate_info['rate_limit'])
            response.headers['X-RateLimit-Remaining'] = str(rate_info['rate_limit'] - rate_info['current_count'])
            response.headers['X-RateLimit-Reset'] = str(rate_info['reset_time'])
