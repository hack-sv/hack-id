    ...
    rows.sort((a, b) => {
        ...
        if (column === "dob" || column === "id") {
            const aNum =
                column === "id"
                    ? parseInt(aVal) || 0
                    : new Date(aVal === "N/A" ? "1900-01-01" : aVal);
            const bNum =
                column === "id"
                    ? parseInt(bVal) || 0
                    : new Date(bVal === "N/A" ? "1900-01-01" : bVal);
    ...
    allRows.forEach((row) => {
        ...
        if (checkedValues.length > 0) {
            if (filterType === "pronouns") {
                ...
                // Map display names back to event IDs for filtering
                const eventIdMap = {};
                for (const [eventId, eventInfo] of Object.entries(eventsData)) {
                    eventIdMap[eventInfo.name] = eventId;
                }
                show = checkedValues.some((displayName) => {
                    const eventId = eventIdMap[displayName];
