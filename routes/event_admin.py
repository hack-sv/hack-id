...
def admin_event_detail(event_id):
    ...
    if not is_valid_event(event_id):
        return render_template("admin/error.html", 
                             error=f"Event '{event_id}' not found"), 404
...
def admin_purge_data_execute():
    ...
    try:
        ...
        if confirmation_1 != "yes":
            return jsonify({
                "success": False, 
                "error": "First confirmation must be 'yes'"
            }), 400
        ...
        if result["success"]:
            if DEBUG_MODE:
                print(f"ADMIN ACTION: {session['user_email']} purged {result['deleted_count']} temporary info records for event {event_id}")
