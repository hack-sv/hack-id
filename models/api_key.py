...
def create_api_key(name, created_by, permissions=None, metadata=None):
    """Create a new API key."""
    conn = get_db_connection()
    api_key = generate_api_key()
    
    conn.execute(
        "INSERT INTO api_keys (name, key, created_by, permissions, metadata) VALUES (?, ?, ?, ?, ?)",
        (name, api_key, created_by, json.dumps(permissions or []), json.dumps(metadata or {}))
    )
    conn.commit()
    conn.close()
...
def get_api_key_logs(key_id=None, limit=10):
    ...
    if key_id:
        ...
        logs = conn.execute(
            "SELECT * FROM api_key_logs ORDER BY timestamp DESC LIMIT ?",
            (limit,)
        ).fetchall()
